...
function sleep(ref,millis){
	...
	if(ref > index){
		...
		setTimeout(function(){
			...
			createView(viewName,width,height);
		},millis);
	}
	return (index<ref);
}
...
// console.log("draw with index : "+index);
 ...
 if(index == 0){
	...
	if(index == 1){
		...
		donut = new JenScript.Donut3D({innerRadius:0,outerRadius:130,thickness : 2, startAngle : 300, tilt:0});
...
function createDonut2D(container, width, height) {
	...
	var donut = new JenScript.Donut2D({innerRadius : 80, outerRadius : 130, startAngleDegree : 45});
